...
const TourDetails: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-vellum">
      ...
      <div className="h-[50vh] lg:h-[85vh] w-full relative group bg-gray-900">
        ...
        {hasMultipleImages && (
          <div className="absolute inset-0 z-20 flex items-center justify-between px-4 lg:px-12 opacity-0 group-hover:opacity-100 transition-opacity">
             <button onClick={prevImage} className="bg-white/10 hover:bg-white/30 p-2 lg:p-4 rounded-full backdrop-blur-md text-white border border-white/20 transition-all"><ChevronLeft size={24} /></button>
             <button onClick={nextImage} className="bg-white/10 hover:bg-white/30 p-2 lg:p-4 rounded-full backdrop-blur-md text-white border border-white/20 transition-all"><ChevronRight size={24} /></button>
          </div>
        )}

        {hasMultipleImages && (
          <div className="absolute bottom-8 lg:bottom-12 left-0 right-0 z-20 flex justify-center gap-2 lg:gap-3">
             {tour.images.map((_, idx) => (
               <button 
                 key={idx} 
                 onClick={() => setCurrentImageIndex(idx)}
                 className={`h-1 lg:h-1.5 rounded-full transition-all duration-300 ${idx === currentImageIndex ? 'w-8 lg:w-10 bg-white' : 'w-2 bg-white/40 hover:bg-white/70'}`}
               />
             ))}
          </div>
        )}
        ...
        <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
          <div className="text-center text-white px-4 max-w-4xl animate-fade-in-up pt-16 lg:pt-20">
            <span className="font-mono text-ochre text-xs lg:text-sm uppercase tracking-[0.2em] mb-4 lg:mb-6 block drop-shadow-md">{tour.duration} • {tour.location}</span>
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-serif font-bold mb-8 drop-shadow-xl leading-tight">{tour.title}</h1>
          </div>
        </div>
      </div>

      <div className="container mx-auto px-6 lg:px-24 py-16 lg:py-24 grid lg:grid-cols-12 gap-12 lg:gap-16">
        <div className="lg:col-span-8 space-y-16 lg:space-y-20">
          <section>
            <div className="flex justify-between items-center mb-8 pb-4 border-b border-gray-200">
               <h2 className="text-2xl lg:text-3xl font-serif font-bold text-savannah-dark">Overview</h2>
            ...
            <p className="text-gray-600 leading-loose font-sans text-base lg:text-lg">{tour.description}</p>
          </section>

          <section>
            <h2 className="text-2xl lg:text-3xl font-serif font-bold mb-10 text-savannah-dark">Itinerary</h2>
            <div className="space-y-12 border-l border-gray-200 ml-3 pl-8 lg:pl-10 relative">
              {tour.itinerary.map((day) => (
                <div key={day.day} className="relative group">
                  <div className="absolute -left-[41px] lg:-left-[49px] top-1.5 w-6 h-6 rounded-full bg-clay group-hover:bg-ochre transition-colors border-4 border-vellum shadow-sm"></div>
                  <h3 className="text-lg lg:text-xl font-bold text-savannah-dark mb-3 font-serif">Day {day.day}: {day.title}</h3>
          ...
          <section className="bg-white p-8 lg:p-10 border border-clay/20 shadow-lg rounded-sm">
            <h2 className="text-xl lg:text-2xl font-serif font-bold mb-8">What's Included</h2>
        ...
        <div className="lg:col-span-4 relative">
          <div className="sticky top-32 bg-white p-8 lg:p-10 shadow-2xl border-t-4 border-savannah-dark rounded-sm">
            <div className="mb-8">
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 p-4 lg:hidden z-40 flex justify-between items-center shadow-[0_-5px_20px_rgba(0,0,0,0.1)]">
         <div>
            <p className="text-[10px] text-gray-500 uppercase tracking-widest mb-1">Starting From</p>
            <p className="font-bold text-lg text-savannah-dark">KES {tour.price.toLocaleString()}</p>
         </div>
         <button onClick={() => setShowModal(true)} className="bg-savannah-dark text-white px-6 py-3 text-xs font-bold uppercase tracking-widest rounded-sm">
            Book Now
         </button>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-vellum w-full max-w-xl p-6 lg:p-10 relative shadow-2xl animate-fade-in-up max-h-[90vh] overflow-y-auto rounded-sm border border-clay">
            <button onClick={() => setShowModal(false)} className="absolute top-4 lg:top-6 right-4 lg:right-6 text-gray-400 hover:text-black transition-colors"><X size={24} /></button>
            <h3 className="text-2xl lg:text-3xl font-serif font-bold mb-2">Plan Your Adventure</h3>
